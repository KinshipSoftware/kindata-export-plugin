...
public class CsvExport extends AbstractBaseModule implements KinOathPanelPlugin {
    ...
    public JScrollPane getUiPanel(PluginDialogHandler dialogHandler, PluginSessionStorage sessionStorage, PluginBugCatcher bugCatcher) throws PluginException {
        ...
        final JTextArea textPane = new JTextArea("This plugin will export the entire project into a CSV file for processing in R.\n"
                + "To use, slect the project then browse for the file into which you wish to export your data and click \"run\".");
        try {
            final String[] databaseList = entityCollection.listDatabases();
            pluginPanel.add(new JComboBox(databaseList), BorderLayout.LINE_START);
